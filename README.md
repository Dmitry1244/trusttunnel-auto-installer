...
28) Расширенное управление: безопасность, маршруты, DNS, мониторинг
0) Выход
```
...
Интерфейс панели построен как рабочая административная консоль: светлая и тёмная темы, группы разделов, `Ctrl+K`, таблица клиентов с поиском, фильтром состояния и страницами, отдельная карточка клиента. Пароли скрыты до явного раскрытия. Обзор обновляет ресурсы и график скорости сетевых интерфейсов каждые 5 секунд; график хранит 60 измерений в текущей вкладке. Это общий трафик VPS, а не расход отдельного клиента.

Панель ориентируется на сценарии 3x-ui, но использует TrustTunnel. Протоколы Xray, REALITY, geosite/geoip, квоты трафика, HWID и недостоверный «онлайн» в ней не имитируются.

### Общие функции панели и терминала

Новые операции используют один Python-обработчик `trusttunnel-panel.py --admin`, поэтому проверки одинаковы в панели и `ttmenu`. Обработчик работает и без запуска веб-сервера; пункт меню скачивает его при необходимости.

| Функция | Панель | ttmenu |
| --- | --- | --- |
| Список, добавление, удаление, случайный пароль, tt:// | Клиенты | 25 |
| Отключение и включение с прежним паролем | Переключатель клиента | 25 → 7 / 8 |
| Заметки, массовое создание с разными паролями | Клиенты | 25 → 9 / 10 |
| Ресурсы, счётчики интерфейсов, журналы и аудит | Обзор / Журналы | 28 → 1–3 |
| Статус UFW, SSH, fail2ban; бан / разбан IP | Безопасность | 28 → 4–6 |
| Настройка числа попыток, окна и срока бана SSH | Безопасность | 28 → 7 |
| Открыть порт / удалить разрешение UFW | Безопасность | 28 → 8 / 9 |
| Direct, WARP, каскад SOCKS5 с проверкой доступности | Маршрутизация | 28 → 10 / 11 |
| Добавить allow/deny по CIDR, удалить отдельное правило | Маршрутизация | 28 → 12 / 13 |
| Сохранить DNS, проверить, отдельно применить к TOML | DNS | 28 → 14–16 |

Отключённые клиенты и заметки хранятся в `/opt/trusttunnel/clients-state.json` с правами `0600` и включаются в backup identity. Отключение убирает пользователя из активного `credentials.toml`, включение возвращает прежний пароль. Изменение доступа перезапускает TrustTunnel, поэтому активные подключения кратковременно переподключаются. Последнего активного клиента удалить или отключить нельзя. Если сервис не запускается, запись credentials и состояния откатывается. При обычном добавлении/изменении клиента профили остальных пользователей не пересоздаются.

Порты SSH, endpoint и панели защищены от закрытия через новые операции UFW. Бан текущего IP администратора в SSH jail отклоняется. Правила CIDR фильтруют **входящий IP клиента**, а не сайты назначения; применяется первое совпадение. Переключение на SOCKS5/WARP сначала проверяет HTTPS-запрос через этот маршрут. Проверка не подтверждает поддержку UDP внешним SOCKS5-прокси.

DNS принимает IP, `https://`, `tls://`, `quic://`. Сохранение не меняет существующие TOML; применение выполняется отдельно. Для IP проверяется DNS-запрос с VPS, для зашифрованных DNS проверяется только разрешение имени резолвера. Доступность DoH/DoT/DoQ из конкретного клиента нужно проверять на устройстве. Новые операции не меняют системный DNS VPS.

Для изменяющих запросов панели используется CSRF-токен; при отсутствии пароля вход закрыт. Для Ubuntu/Debian с Python до 3.11 установщик добавляет `python3-tomli`. Для DNS-диагностики используется `dnsutils`.

AntiDPI и post-quantum TLS по умолчанию выключены. В расширенном блоке TLS сохранение пересобирает экспортируемые TOML-файлы, но не изменяет сертификат, логины или пароли. Обычное сохранение DNS не пересобирает профили. Сначала проверь новый TOML на одном устройстве.
