...
setup_env(){
  ...
  if [[ -n "${current_password:-}" ]]; then
    read -r -p "Сохранить текущий пароль панели? [Y/n]: " keep_pw
    keep_pw="${keep_pw:-Y}"
    if [[ "$keep_pw" =~ ^[Nn]$ ]]; then
      admin_password="$(ask_secret 'Новый пароль панели')"
    else
      admin_password="$current_password"
    fi
  else
    admin_password="$(ask_secret 'Пароль панели')"
    [[ -n "$admin_password" ]] || { err "Пароль не может быть пустым"; return 1; }
  fi

  if [[ -n "${current_secret:-}" ]]; then
    panel_secret="$current_secret"
  else
    panel_secret="$(gen_secret)"
  fi
...
maybe_open_firewall(){
  ...
  if command -v ufw >/dev/null 2>&1; then
    read -r -p "Открыть порт $port/tcp в UFW для веб-панели? [y/N]: " ans
    ans="${ans:-N}"
    if [[ "$ans" =~ ^[Yy]$ ]]; then
      ufw allow "$port/tcp"
